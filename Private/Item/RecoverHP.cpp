...
ARecoverHP::ARecoverHP()
{
    ...
    ItemType = EItemType::RECOVER_HP;
}


// ゲーム開始時に必要な参照・イベント・初期状態を設定する。
void ARecoverHP::BeginPlay()
{
	Super::BeginPlay();
    InitializeItem("RecoverHP");
}

// 構成要素の生成完了後に、相互参照とイベントを接続する。
void ARecoverHP::PostInitializeComponents()
{
    Super::PostInitializeComponents();

    SphereItem->OnComponentBeginOverlap.AddDynamic(this, &ARecoverHP::OnOverlapsBegin);
}


// 毎フレーム、経過時間に応じて継続中の状態と表示を更新する。
void ARecoverHP::Tick(float DeltaTime)
{
...
void ARecoverHP::OnOverlapsBegin_Implementation(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
    ...
    case ECC_GameTraceChannel4:
    {
        ...
        if (AMyPlayer* Player = Cast<AMyPlayer>(OtherActor))
        {
            ...
            if(Player->GetAttributeSet()->GetHp() >= Player->GetAttributeSet()->GetMaxHp())
            {
                UE_LOG(LogTemp, Warning, TEXT("Player HP is full, skipping heal effect."));
                return;
			}
