...
AIncreaseDef::AIncreaseDef()
{
	...
	ItemType = EItemType::INCREASE_DEF;
}


// ゲーム開始時に必要な参照・イベント・初期状態を設定する。
void AIncreaseDef::BeginPlay()
{
	Super::BeginPlay();
    InitializeItem("IncreaseDef");
}


// 毎フレーム、経過時間に応じて継続中の状態と表示を更新する。
void AIncreaseDef::Tick(float DeltaTime)
{
...
void AIncreaseDef::PostInitializeComponents()
{
    Super::PostInitializeComponents();

    SphereItem->OnComponentBeginOverlap.AddDynamic(this, &AIncreaseDef::OnOverlapsBegin);
}

// 重なりを検知した相手の種類を確認し、該当するダメージまたは取得効果を適用する。
void AIncreaseDef::OnOverlapsBegin_Implementation(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
    ...
    case ECC_GameTraceChannel4:
    {
        ...
        if (AMyPlayer* Player = Cast<AMyPlayer>(OtherActor))
        {
            ...
            Player->IncreasePlayerDef(Amount);
