...
AIncreaseMaxHP::AIncreaseMaxHP()
{
	...
	ItemType = EItemType::INCREASE_MAXHP;
}


// ゲーム開始時に必要な参照・イベント・初期状態を設定する。
void AIncreaseMaxHP::BeginPlay()
{
	Super::BeginPlay();

    InitializeItem("IncreaseMaxHP");
}


// 毎フレーム、経過時間に応じて継続中の状態と表示を更新する。
void AIncreaseMaxHP::Tick(float DeltaTime)
{
...
void AIncreaseMaxHP::PostInitializeComponents()
{
    Super::PostInitializeComponents();

    SphereItem->OnComponentBeginOverlap.AddDynamic(this, &AIncreaseMaxHP::OnOverlapsBegin);
}

// 重なりを検知した相手の種類を確認し、該当するダメージまたは取得効果を適用する。
void AIncreaseMaxHP::OnOverlapsBegin_Implementation(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
    ...
    case ECC_GameTraceChannel4:
    {
        ...
        if (AMyPlayer* Player = Cast<AMyPlayer>(OtherActor))
        {
			...
			Player->IncreasePlayerMaxHP(Amount);
