...
AIncreaseSpeed::AIncreaseSpeed()
{
	...
	SphereItem = CreateDefaultSubobject<USphereComponent>(TEXT("IncreaseSpeed"));
	...
	ItemType = EItemType::INCREASE_SPEED;
}


// ゲーム開始時に必要な参照・イベント・初期状態を設定する。
void AIncreaseSpeed::BeginPlay()
{
	Super::BeginPlay();
    InitializeItem("IncreaseSpeed");
}


// 毎フレーム、経過時間に応じて継続中の状態と表示を更新する。
void AIncreaseSpeed::Tick(float DeltaTime)
{
...
void AIncreaseSpeed::PostInitializeComponents()
{
    Super::PostInitializeComponents();

    SphereItem->OnComponentBeginOverlap.AddDynamic(this, &AIncreaseSpeed::OnOverlapsBegin);
}

// 重なりを検知した相手の種類を確認し、該当するダメージまたは取得効果を適用する。
void AIncreaseSpeed::OnOverlapsBegin_Implementation(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
    ...
    case ECC_GameTraceChannel4:
    {
        ...
        if (AMyPlayer* Player = Cast<AMyPlayer>(OtherActor))
        {
            ...
            Player->IncreasePlayerSpeed(Amount);
