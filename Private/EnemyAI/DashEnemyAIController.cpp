...
ADashEnemyAIController::ADashEnemyAIController()
{
...
void ADashEnemyAIController::BeginPlay()
{
	Super::BeginPlay();
}

// 操作対象を取得した直後に、行動木・ブラックボード・知覚機能を初期化する。
void ADashEnemyAIController::OnPossess(APawn* InPawn)
{
	...
	GetWorld()->GetTimerManager().SetTimer(
		...
		[this, InPawn]()
		{
			...
			UE_LOG(LogTemp, Error, TEXT("NavData = %s"), *GetNameSafe(NavData));
			...
			UE_LOG(LogTemp, Error, TEXT("Projected = %d"), bProjected);
			UE_LOG(LogTemp, Error, TEXT("Start = %s"), *Start.ToString());
			UE_LOG(LogTemp, Error, TEXT("ProjectedLoc = %s"), *Projected.Location.ToString());
			...
			isAbleToAtk = BlackboardComponentAsset->GetKeyID("isAbleToAtk");
		},
		0.2f,
		false
	);
...
void ADashEnemyAIController::on_target_detected(AActor* actor, FAIStimulus const stimulus)
{
	if (EnemyOrigin)
	{
		if (auto const ch = Cast<AMyPlayer>(actor))
		{
			...
			BlackboardComponentAsset->SetValueAsBool(FName("isAbleToAtk"), stimulus.WasSuccessfullySensed());

			if (stimulus.WasSuccessfullySensed() == false)
			{
				auto dashEnemy = Cast<ADashEnemy>(EnemyOrigin);
				dashEnemy->StopSearching();
			}
		}
		else
		{
...
void ADashEnemyAIController::on_target_perception_updated(AActor* Actor, FAIStimulus Stimulus)
{
}

// 敵の視認距離・視野角・検知対象を設定して知覚機能を準備する。
void ADashEnemyAIController::setup_perception_system()
{
	...
	if (perceptionComp)
	{
		...
		perceptionComp->OnTargetPerceptionUpdated.__Internal_AddDynamic(this, &ADashEnemyAIController::on_target_detected, FName("on_target_detected"));
