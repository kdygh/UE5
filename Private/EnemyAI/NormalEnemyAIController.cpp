...
void ANormalEnemyAIController::on_target_detected(AActor* actor, FAIStimulus const stimulus)
{
	if (EnemyOrigin)
	{
...
// 視界を指定された値へ更新する。
void ANormalEnemyAIController::SetSightRadius(bool isDetected)
{
	SightConfig->PeripheralVisionAngleDegrees = (isDetected == true) ? 180.f : 60.f;
}

// ゲーム開始時に必要な参照・イベント・初期状態を設定する。
void ANormalEnemyAIController::BeginPlay()
{
	Super::BeginPlay();
}
...
void ANormalEnemyAIController::setup_perception_system()
{
	...
	if (perceptionComp)
	{
		...
		perceptionComp->OnTargetPerceptionUpdated.__Internal_AddDynamic(this, &ANormalEnemyAIController::on_target_detected, FName("on_target_detected"));
