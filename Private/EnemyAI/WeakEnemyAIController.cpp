...
AWeakEnemyAIController::AWeakEnemyAIController()
{
...
void AWeakEnemyAIController::on_target_detected(AActor* actor, FAIStimulus const stimulus)
{
	if (EnemyOrigin)
	{
...
// 視界を指定された値へ更新する。
void AWeakEnemyAIController::SetSightRadius(bool isDetected)
{
	if(SightConfig == nullptr)
	{
		UE_LOG(LogTemp, Warning, TEXT("SightConfig is nullptr"));
		return;
	}
...
void AWeakEnemyAIController::BeginPlay()
{
	Super::BeginPlay();
}

// 操作対象を取得した直後に、行動木・ブラックボード・知覚機能を初期化する。
void AWeakEnemyAIController::OnPossess(APawn* InPawn)
{
...
void AWeakEnemyAIController::setup_perception_system()
{
	...
	if (perceptionComp)
	{
		...
		perceptionComp->OnTargetPerceptionUpdated.__Internal_AddDynamic(this, &AWeakEnemyAIController::on_target_detected, FName("on_target_detected"));
